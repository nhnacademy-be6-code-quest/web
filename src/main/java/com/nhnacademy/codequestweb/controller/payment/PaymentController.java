...
public class PaymentController {
    ...
    public String savePayment(@RequestHeader HttpHeaders headers, Model model, @RequestParam("tossOrderId") String tossOrderId, HttpServletRequest req) {

        headers.set("access", CookieUtils.getCookieValue(req, "access"));

//      1. 주문에서 받은 값을 토대로 사용자에게 보여 주기
        PaymentOrderShowRequestDto paymentOrderShowRequestDto = paymentService.findPaymentOrderShowRequestDtoByOrderId(headers, tossOrderId);
...
//        2. 결제 검증 및 승인 창에서 필요한 요소를 Order 에서 받아 오기 : @RequestHeader 로 해결함. // 303 ->
        ...
        PaymentOrderApproveRequestDto paymentOrderApproveRequestDto = paymentService.findPaymentOrderApproveRequestDtoByOrderId(headers, tossOrderId);
...
//        3. 조작 확인하기 : 주문 정보가 일치하지 않으면 실패 페이지로 이동하기.
        ...
        TossPaymentsResponseDto tossPaymentsResponseDto = paymentService.approvePayment(headers, tossOrderId, amount, paymentKey);
    ...
    public String postProcessPayment(@PathVariable("tossOrderId") String tossOrderId, HttpServletRequest request, RedirectAttributes redirectAttributes){

        HttpHeaders headers = new HttpHeaders();
        headers.set("access", CookieUtils.getCookieValue(request, "access"));

        // 5-3) 포인트 적립하기
        PostProcessRequiredPaymentResponseDto postProcessRequiredPaymentResponseDto = paymentService.getPostProcessRequiredPaymentResponseDto(tossOrderId);

        boolean successAccumulatePoint = accumulatePoint(headers, postProcessRequiredPaymentResponseDto);
    ...
    private boolean accumulatePoint(HttpHeaders headers, PostProcessRequiredPaymentResponseDto postProcessRequiredPaymentResponseDto){
        try {
            // 포인트 적립하기
            paymentService.accumulatePoint(headers, postProcessRequiredPaymentResponseDto.getAmount());
            return true;
        } catch (Exception e) {
